...
public class Step2and3 {
    private static final Logger log = LoggerFactory.getLogger(Step2and3.class);
...
//    static ScheduledThreadPoolExecutor step1 = null;
     ...
     * 2.处理2：启动线程B，用户监视邮箱是否有新的邮件，并获取邮箱中的URL放入注册queue
    ...
    public static void main(String[] args) {
        ...
        Step2and3.loopCount = Integer.parseInt(InitPropertiesUtil.getInitProperty("main.thread.loop.max.count"));
...
////        parseHtml(InitPropertiesUtil.getInitProperty("handle1.businessTestB.url"), eventList);
//        parseHtml(InitPropertiesUtil.getInitProperty("main.businessA.url"), eventList);
//        parseHtml(InitPropertiesUtil.getInitProperty("main.businessB.url"), eventList);
//        parseHtml(InitPropertiesUtil.getInitProperty("main.businessC.url"), eventList);
//        
//
//        if (eventList.size() == 0) {
...
//        }
        ...
        int eventListSize = 3; // 账户一天订购数量
        Handler2 handler2 = new Handler2(queue, handler2Count, eventListSize);
...
//                step3.scheduleWithFixedDelay(new Handler3(queue), 5, 10, TimeUnit.SECONDS);
            ...
            if (handler2Count.get() >= eventListSize) {
                step2.shutdown();
                if (step2.isShutdown()) {
                ...
                log.info("未处理邮件数目: " + queue.size() + "，已发送邮件数：" + handler2Count.get() + "，需发送邮件数：" + eventListSize);
            }
            try {
...
class Step2and3T1 implements Runnable{
    ...
    public Step2and3T1(String eventId, String eventType) {
    ...
    public void run() {
        // 隔N秒发送一次请求
        step1.scheduleWithFixedDelay(new Handler1(eventId, eventType, count), 0, 5, TimeUnit.SECONDS);
//        step1.scheduleWithFixedDelay(new Handler1(eventId, eventType, count), 5, 10, TimeUnit.SECONDS);
        while (true) {
            if (count.get() > Step2and3.loopCount) {
                step1.shutdown();
                if (step1.isShutdown()) {
