	...
	private void step1(String email){
		...
		try{
			...
			if(page.sendEmailVerify.isDisplayed()){
				System.out.println("url1 index "+index+"  发送成功");
			}
		}catch(Exception ex){
			try{
				if(page.error.getText().equals("DB接続に失敗しました。")){
					System.out.println("url1 index "+index+"  DB接続に失敗しました。");
					driver.get(driver.getCurrentUrl());
					page.email1.sendKeys(email);
					page.email2.sendKeys(email);
					step1(email);
				}else{
					System.out.println("url1 index: "+index+"  step1 error.");
				}
			}catch(Exception ex1){
				System.out.println("url1  index: "+index+"  step1 exception.");
			}
		}
	}
}
